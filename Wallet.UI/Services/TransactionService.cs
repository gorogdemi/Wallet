...
namespace Wallet.UI.Services
{
    ...
    {
        private const string ApiUrl = "api/transaction";
        ...
        public Task<TransactionResponse> GetAsync(int id) => _httpClient.GetFromJsonAsync<TransactionResponse>(ApiUrl);

        public async Task<bool> UpdateAsync(int id, TransactionRequest request) => (await _httpClient.PutAsJsonAsync(ApiUrl, request)).IsSuccessStatusCode;
    }
}